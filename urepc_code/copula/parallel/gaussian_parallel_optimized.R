...
solve_column_g <- function(K,n,x,z,sampleOfG,mu,sigma,j){
  ...
  kweightshat <- solve.QP.compact(Dmat, dvec, Amat,Aind, bvec, meq=3)$solution # 'k' like kernel
...
parallel_fun_1 <- function (xtilde,n,j,z,x,mu,sigma){
  ...
  kweightshat <- solve.QP.compact(Dmat, dvec, Amat,Aind, bvec, meq=3)$solution
...
EMalgo <- function(
  ...
  time_init <- system.time(
    result <- mclapply(1:d,across_dimensions_aux, mc.cores = numCores)
  )
  for(i in 1:d){
    G[,i,] <- result[[i]][,,1]
    g[,i,] <- result[[i]][,,2]
...
# saveRDS(result, file = "opti_data/result.RDS")
# end_tm <- Sys.time()
# print(paste(
#   "Taken:",
#   end_tm - start_tm,
