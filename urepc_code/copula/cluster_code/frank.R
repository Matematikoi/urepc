...
EMalgo <- function(
  ...
  for(j in 1:d){
    ...
    for(z in 1:K){
      ...
      command <- paste(
        ...
        outside_resource,
        "--job-name=opti_key -o cluster_out_data/out_%j_.out run_optimization.sh -i",
        in_file,
        " -o ",
        out_file,
        sep= " "
  ...
  while (system("squeue -u glozanop -n opti_key | wc -l",intern = TRUE) != "1"){
    Sys.sleep(5)
    elements_left <- system("squeue -u glozanop -n opti_key | wc -l", intern = TRUE)
    cat(elements_left,",")
  }
  for (j in 1:d){
    for(z in 1:K){
  ...
  for(z in 1:K){
    print(paste("processing theta_z: ", z))
    theta[z] <- max(0,fitCopula(cop[[z]], pobs(x[km$cluster==z,]),
                          method="mpl")@estimate) #theta[,z]
  ...
  for(t in 1:nbit){
    ...
    for(z in 1:K){
      for(i in 1:n){
        num[i,z] <- abs(dCopula(G[i,,z],copula=cop[[z]])*prod(g[i,,z])*pz[z] ) 
      }
    }
    if(debug){
    ...
    while (system("squeue -u glozanop -n opti_key | wc -l",intern = TRUE) != "1"){
      Sys.sleep(5)
      items_left <- system("squeue -u glozanop -n opti_key | wc -l",intern = TRUE)
      cat (items_left, ",")
    }
    for (j in 1:d){
      for(z in 1:K){
    ...
    if(commonCopula){ 
      ...
      for(z in 1:K){
        print(paste("updating theta_z: ",z))
        ...
        try(newTheta[z] <- optim(
          ...
          optimFoo,
          z=z,
          method="Brent",# or Brent #or L-BFGS-B
          lower = 0,
          upper = 50
        )$par)
        # }
...
sampleSize <- 1000
nb_it_em <- 10
outside_resource <- " --time=0:23:00 --mem-per-cpu=23G"
